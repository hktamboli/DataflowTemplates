...
import com.google.cloud.teleport.v2.neo4j.model.job.GlobalJobSettings;
...
public class GoogleCloudToNeo4j {
    ...
    private final GlobalJobSettings globalSettings;
    ...
    public GoogleCloudToNeo4j(Neo4jFlexTemplateOptions pipelineOptions) {
        ...
        globalSettings = GlobalJobSettings.wrap(importSpecification.getConfiguration());
        ...
        for (org.neo4j.importer.v1.sources.Source source : importSpecification.getSources()) {
            ...
            Provider providerImpl = ProviderFactory.of(source);
    ...
    public void run() {
        ...
        try (Neo4jConnection directConnect =
            ...
            boolean resetDb = globalSettings.resetDatabase();
            if (!resetDb) {
        ...
        for (var source : importSpecification.getSources()) {
            ...
            // get provider implementation for source
            Provider<? extends Source> provider = ProviderFactory.of(source);
            ...
            if (!targetsHaveTransforms || !provider.supportsSqlPushDown()) {
                nullableSourceBeamRows =
                        pipeline
                                .apply(
                                        "Query " + source.getName(), provider.querySourceBeamRows(sourceBeamSchema))
            ...
            for (NodeTarget nodeTarget : nodeTargets) {
                TargetQuerySpec<NodeTarget> targetQuerySpec =
                        new TargetQuerySpecBuilder<NodeTarget>()
                                .sourceBeamSchema(sourceBeamSchema)
                                .nullableSourceRows(nullableSourceBeamRows)
                                .target(nodeTarget)
                                .build();
                String nodeStepDescription =
                        targetSequence.getSequenceNumber(nodeTarget)
                        + ": "
                        + source.getName()
                        + "->"
                        + nodeTarget.getName()
                ...
                Neo4jRowWriterTransform targetWriterTransform =
                        new Neo4jRowWriterTransform(importSpecification, neo4jConnection, templateVersion, nodeTarget);
            ...
            List<Target> relationshipTargets =
                    importSpecification.getActiveTargetsBySourceAndType(sourceName, TargetType.edge);
            for (Target relationshipTarget : relationshipTargets) {
                ...
                Neo4jRowWriterTransform targetWriterTransform =
                        new Neo4jRowWriterTransform(
                                importSpecification, neo4jConnection, templateVersion, relationshipTarget);
            ...
            List<Target> customQueryTargets =
                    importSpecification.getActiveTargetsBySourceAndType(sourceName, TargetType.custom_query);
            for (Target customQueryTarget : customQueryTargets) {
                ...
                Neo4jRowWriterTransform targetWriterTransform =
                        new Neo4jRowWriterTransform(
                                importSpecification, neo4jConnection, templateVersion, customQueryTarget);
        ...
        runBeamActions(importSpecification.getPostloadActions(), processingQueue);
    ...
    private <T extends Target> List<T> getActiveTargetsBySourceAndType(ImportSpecification importSpecification, String sourceName, Class<T> targetType) {
        ...
        throw new IllegalArgumentException(String.format("Cannot filter unsupported target type %s", targetType));
    }

    private static <T extends Target> List<T> findActiveTargetBySource(String sourceName, List<T> targets) {
        return targets
                .stream()
