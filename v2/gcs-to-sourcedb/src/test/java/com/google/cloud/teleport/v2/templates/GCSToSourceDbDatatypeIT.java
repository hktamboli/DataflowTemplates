...
public class GCSToSourceDbDatatypeIT extends GCSToSourceDbITBase {
  ...
  @Test
  public void gcsToSourceDataTypeConversionTest() throws IOException, InterruptedException {
  ...
  private void writeRowsInSpanner() {
    ...
    Mutation m =
        Mutation.newInsertOrUpdateBuilder(TABLE1)
            ...
            .set("longblob_column")
            .to(Value.bytes(ByteArray.copyFrom("largeblob_column_value")))
            .set("enum_column")
            .to("2")
            .set("bool_column")
            ...
            .set("binary_column")
            .to(Value.bytes(ByteArray.copyFrom("binary_column_value")))
            .set("varbinary_column")
            .to(Value.bytes(ByteArray.copyFrom("varbinary_column_value")))
            .set("bit_column")
            .to(Value.bytes(ByteArray.copyFrom("bit_col")))
            .build();
  ...
  private void assertRowInMySQL() throws InterruptedException {
    ...
    for (int i = 0; rowCount != 1 && i < 60; ++i) {
      rowCount = jdbcResourceManagers.get(0).getRowCount(TABLE1);
    ...
    assertThat(row.get("text_column")).isEqualTo("text_byte");
    assertThat(row.get("date_column")).isEqualTo("2024-02-08T00:00:00Z");
    ...
    assertThat(row.get("float_column")).isEqualTo(45.67);
    assertThat(row.get("double_column")).isEqualTo(123.789);
    assertThat(row.get("decimal_column")).isEqualTo(456.12);
    assertThat(row.get("datetime_column")).isEqualTo("2024-02-08T08:15:30Z");
    assertThat(row.get("timestamp_column")).isEqualTo("2024-02-08T08:15:30Z");
    assertThat(row.get("time_column")).isEqualTo(29730000000l);
    assertThat(row.get("year_column")).isEqualTo(2022);
    assertThat(row.get("char_column")).isEqualTo("char_byte");
    assertThat(row.get("tinytext_column")).isEqualTo("tinytext_byte");
    assertThat(row.get("mediumtext_column")).isEqualTo("mediumtext_byte");
    assertThat(row.get("longtext_column")).isEqualTo("longtext_byte");
    assertThat(row.get("tinyblob_column")).isEqualTo("74696e79626c6f625f646174615f31");
    assertThat(row.get("blob_column")).isEqualTo("626c6f625f646174615f31");
    assertThat(row.get("mediumblob_column")).isEqualTo("6d656469756d626c6f625f646174615f31");
    assertThat(row.get("longblob_column")).isEqualTo("6c6f6e67626c6f625f646174615f31");
    assertThat(row.get("enum_column")).isEqualTo(2);
    assertThat(row.get("bool_column")).isEqualTo(0);
    assertThat(row.get("other_bool_column")).isEqualTo(1);
    assertThat(row.get("binary_column")).isEqualTo("62696e6172795f31");
    assertThat(row.get("varbinary_column")).isEqualTo("76617262696e6172795f646174615f31");
    assertThat(row.get("bit_column")).isEqualTo(102);
  }
