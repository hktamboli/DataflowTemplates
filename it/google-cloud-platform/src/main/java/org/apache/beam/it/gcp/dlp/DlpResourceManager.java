...
public class DlpResourceManager implements ResourceManager {
  ...
  private final Map<DlqTemplate, String> createdTemplates = new HashMap<>();
  ...
  public DeidentifyTemplate createDeidentifyTemplate(DeidentifyTemplate template)
    ...
    try (DlpServiceClient client = getDlpClient()) {
      ...
      createdTemplates.put(DlqTemplate.DEIDENTIFY, deidentifyTemplate.getName());
  ...
  public InspectTemplate createInspectTemplate(InspectTemplate template) throws IOException {
    try (DlpServiceClient client = getDlpClient()) {
      ...
      createdTemplates.put(DlqTemplate.INSPECT, inspectTemplate.getName());
  ...
  public void cleanupAll() {
    createdTemplates.forEach(
        (dlqTemplateType, templateName) -> {
          try {
            if (dlqTemplateType.equals(DlqTemplate.DEIDENTIFY)) {
